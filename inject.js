...
chrome.runtime.sendMessage({greeting:"requestAll"}, function (reply) {
    ...
    var courses = reply.courses;
    var preferences = reply.preferences;
    ...
    if (PROBLEM) {
    	...
    	if (percentage !== null) {
    		...
    		// compare to the percentage in the courses object.  Use the ABS thing because the page rounds to the whole number, not down
    		 if (Math.abs(courses[CLASS][ASSIGNMENT]["problems"][PROBLEM-1] - percentage) > 1) {
    		 	courses[CLASS][ASSIGNMENT]["problems"][PROBLEM-1] = percentage;

    		 	// recalculate average
    		 	var avg = 0;
    		 	for (var i = 0; i < courses[CLASS][ASSIGNMENT]["problems"].length; i++) {
    		 		avg += courses[CLASS][ASSIGNMENT]["problems"][i];
    		 	}
    		 	// update average
    		 	courses[CLASS][ASSIGNMENT]["average"] = Math.floor(avg/courses[CLASS][ASSIGNMENT]["problems"].length);
    		 	updateCourses = true;
    		 }
    	}
        ...
        if (tableRows !== undefined) {
	        ...
	        if (courses[CLASS][ASSIGNMENT] === undefined) {
        	    courses[CLASS][ASSIGNMENT] = {
        		    "problems": [],	// one-indexed array where the key is the percentage completed of a problem (index is the problem name)
        		    "average": 0	// average of all the values inside of the problems [] array
        	    };
        	    updateCourses = true;
            }
            ...
            if (problem_array.length !== courses[CLASS][ASSIGNMENT]["problems"].length) {
        	    courses[CLASS][ASSIGNMENT]["problems"] = problem_array;
        	    courses[CLASS][ASSIGNMENT]["average"] = Math.floor(total / problem_array.length);
        	    updateCourses = true;
            } else {
        	    for (var i = 0; i < problem_array.length; i++) {
        		    if (problem_array[i] !== courses[CLASS][ASSIGNMENT]["problems"][i]) {
        			    courses[CLASS][ASSIGNMENT]["problems"] = problem_array;
        			    courses[CLASS][ASSIGNMENT]["average"] = Math.floor(total / problem_array.length);
        			    updateCourses = true;
        			    break;
        ...
        if (tableRows !== undefined) {
    	    ...
    	    for (var i = 1; i < tableRows.length; i++) { // skip the first row (header)
        	    ...
        	    if (courses[CLASS][assignmentName] === undefined) {
        		    ...
        		    if (courses[CLASS][assignmentName]["problems"].length > 0) {
        			    // choose the class depending on the percentage
        			    var className = "WWsuccess";
        			    if (courses[CLASS][assignmentName]["average"] < 70) {
        				    className = "WWdanger";
        			    } else if (courses[CLASS][assignmentName]["average"] < 100) {
        				    className = "WWwarning";
        			    }
        			    // get the completion percentage and append it to the table

        			    tableRows[i].children[2].innerHTML += '<span class="'+className+' completionPercentage">' +courses[CLASS][assignmentName]["average"]+'</span>';
                    }
