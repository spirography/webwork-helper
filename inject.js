...
/*
 * Calculates the size of a class inside of the courses variable
 * all numbers in JavaScript are 8 bytes
 */
function sizeOfClass(nameOfClass) {
    var bytes = 0;
    for (var key in courses[nameOfClass]) {
        if (courses[nameOfClass][key]["a"] === 100) { // no problems array
            bytes += 8;
        } else {
            bytes += 8*courses[nameOfClass][key]["p"].length + 8;
        }
    }
    return bytes;
}
